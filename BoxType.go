...
func (e *Encoder) box(v *Box) (err os.Error){
     
     if _, _, error := e.t.PointerCheck(unsafe.Pointer(v), "92b880b703551e6d142722be0dc2bd870faf844d453bf60797ab0ebae1a4d5387281794425f4e74ecc7010a48c8270c05e115526d2b65e2e474c8cbd3fdf3e18", uint64(unsafe.Sizeof(*v))); error == nil {
...
func (e *Encoder) boxInternal(v *Box) (err os.Error){
...
func (d *Decoder) box() (v *Box, error os.Error){
...
func (d *Decoder) boxInternal() (v *Box, error os.Error){
...
func (e *Encoder) any(a Any) (err os.Error){
     ...
     case Box:
     	hashByte, err := hex.DecodeString("92b880b703551e6d142722be0dc2bd870faf844d453bf60797ab0ebae1a4d5387281794425f4e74ecc7010a48c8270c05e115526d2b65e2e474c8cbd3fdf3e18")
	if err != nil {
	   return err
	}
     	// hashByte := []byte("92b880b703551e6d142722be0dc2bd870faf844d453bf60797ab0ebae1a4d5387281794425f4e74ecc7010a48c8270c05e115526d2b65e2e474c8cbd3fdf3e18")
     	// e.string("92b880b703551e6d142722be0dc2bd870faf844d453bf60797ab0ebae1a4d5387281794425f4e74ecc7010a48c8270c05e115526d2b65e2e474c8cbd3fdf3e18")
...
func (d *Decoder) any() (retValue *Any, error os.Error) {
     ...
     case bytes.Equal(encodedHash, []byte("92b880b703551e6d142722be0dc2bd870faf844d453bf60797ab0ebae1a4d5387281794425f4e74ecc7010a48c8270c05e115526d2b65e2e474c8cbd3fdf3e18")):
...
func (t *Box) IpcWrite(serviceName string, hostname string) (syscall.Fd, syscall.Status) {
	serviceFd, status := goodmiddleman.OpenDirectoryPath("/services/" + serviceName)
	if status != syscall.StatusOk {
		log.Fatalf ("Error calling OpenDirectory /services/%s: %v\n", serviceName, status)
		return 0, status
	...
	netFd, status := goodmiddleman.IpcRepeat(serviceFd, sName, hostname, nil)
	if status != syscall.StatusOk {
		log.Fatalf ("Error calling IpcWriteBox: %v\n", status)
		return 0, status
	}
	writer := goodmiddleman.NewWriter(netFd)
	goodmiddleman.Close(serviceFd)
...
func (t *Box) Write(fd syscall.Fd) (syscall.Status) {
...
func (t *Box) WriteVar(path string) (syscall.Status) {
...
func (t *Box) Read(fd syscall.Fd) (syscall.Status) {
...
func (t *Box) ReadVar(path string) (syscall.Status) {
	fd, name, status := goodmiddleman.OpenPathLastDirectory(path)
	if status != syscall.StatusOk {
		return status
	}
...
func (t *Box) CreateDirectory(fd syscall.Fd, name string, label string) (syscall.Status) {
       hash := goodmiddleman.HashValue{ 0x92,0xb8,0x80,0xb7,0x03,0x55,0x1e,0x6d,0x14,0x27,0x22,0xbe,0x0d,0xc2,0xbd,0x87,0x0f,0xaf,0x84,0x4d,0x45,0x3b,0xf6,0x07,0x97,0xab,0x0e,0xba,0xe1,0xa4,0xd5,0x38,0x72,0x81,0x79,0x44,0x25,0xf4,0xe7,0x4e,0xcc,0x70,0x10,0xa4,0x8c,0x82,0x70,0xc0,0x5e,0x11,0x55,0x26,0xd2,0xb6,0x5e,0x2e,0x47,0x4c,0x8c,0xbd,0x3f,0xdf,0x3e,0x18, }  
       return goodmiddleman.CreateDirectory(fd, name, label, hash)
}

func (t *Box) CreateDirectoryPath(path string, label string) (syscall.Status) {
       hash := goodmiddleman.HashValue{ 0x92,0xb8,0x80,0xb7,0x03,0x55,0x1e,0x6d,0x14,0x27,0x22,0xbe,0x0d,0xc2,0xbd,0x87,0x0f,0xaf,0x84,0x4d,0x45,0x3b,0xf6,0x07,0x97,0xab,0x0e,0xba,0xe1,0xa4,0xd5,0x38,0x72,0x81,0x79,0x44,0x25,0xf4,0xe7,0x4e,0xcc,0x70,0x10,0xa4,0x8c,0x82,0x70,0xc0,0x5e,0x11,0x55,0x26,0xd2,0xb6,0x5e,0x2e,0x47,0x4c,0x8c,0xbd,0x3f,0xdf,0x3e,0x18, }
